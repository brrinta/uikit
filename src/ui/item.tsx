...
export const itemVariants = cvaWithMeta(
	`[a]:hover:bg-muted rounded-md border text-sm w-full group/item focus-visible:border-ring focus-visible:ring-ring/50 flex items-center
	 flex-wrap outline-none transition-colors duration-100 focus-visible:ring-[3px] [a]:transition-colors`,
	{
		variants: {
			variant: {
...
export const itemMediaVariants = cvaWithMeta(
	`gap-2 group-has-[[data-slot=item-description]]/item:translate-y-0.5 group-has-[[data-slot=item-description]]/item:self-start flex shrink-0
...
function ItemDescription({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p
			...
			className={cn(
				`text-muted-foreground text-left text-sm leading-normal group-data-[size=xs]/item:text-xs [&>a:hover]:text-primary line-clamp-2
				font-normal [&>a]:underline [&>a]:underline-offset-4`,
				className,
			)}
			{...props}
		/>
...
const ItemComponent = Item as CompoundItem;
ItemComponent.Media = ItemMedia;
ItemComponent.Content = ItemContent;
ItemComponent.Actions = ItemActions;
ItemComponent.Group = ItemGroup;
ItemComponent.Separator = ItemSeparator;
ItemComponent.Title = ItemTitle;
ItemComponent.Description = ItemDescription;
ItemComponent.Header = ItemHeader;
ItemComponent.Footer = ItemFooter;

export { ItemComponent as Item, ItemMedia, ItemContent, ItemActions, ItemGroup, ItemSeparator, ItemTitle, ItemDescription, ItemHeader, ItemFooter };
