...
const MASK_PATTERNS: Record<MaskPatternKey, MaskPattern> = {
	...
	date: {
		...
		validate: (value) => {
			...
			const maxDays = month === 2 && ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0) ? 29 : (daysInMonthCache[month - 1] ?? 31);
...
function applyMask(opts: { value: string; pattern: string; currency?: string; locale?: string; mask?: MaskPatternKey | MaskPattern }): string {
...
function MaskInput(props: MaskInputProps) {
	...
	const onValueChange = React.useCallback(
		...
		[maskPattern, isControlled, onValueChangeProp, onValidate, onInputValidate, composing, shouldValidate, withoutMask, transformOpts, mask, value],
	);
	...
	const onPaste = React.useCallback(
		...
		[onPasteProp, withoutMask, maskPattern, mask, transformOpts, isControlled, shouldValidate, onInputValidate, onValueChangeProp],
	);
	...
	return (
		...
		transition-[color,box-shadow] selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-7 file:border-0 file:bg-transparent
		 file:font-medium file:text-foreground file:text-sm placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed
		  disabled:opacity-50 md:text-sm dark:bg-input/30 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50
