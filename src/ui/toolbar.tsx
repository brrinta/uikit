...
function ToolbarLink({ className, ...props }: React.ComponentProps<typeof BaseToolbar.Link>) {
	return (
		<BaseToolbar.Link
			className={cn(
				`text-muted-foreground hover:text-foreground focus-visible:ring-ring/50 inline-flex h-9 shrink-0 items-center gap-2 rounded-md px-4
				 text-sm no-underline transition-colors focus-visible:ring-[3px] focus-visible:outline-none [&_svg]:pointer-events-none
...
// Compound export
type CompoundToolbar = typeof Toolbar & {
...
const ToolbarComponent = Toolbar as CompoundToolbar;
ToolbarComponent.Button = ToolbarButton;
ToolbarComponent.Separator = ToolbarSeparator;
ToolbarComponent.Input = ToolbarInput;
ToolbarComponent.Group = ToolbarGroup;
ToolbarComponent.Link = ToolbarLink;
export { ToolbarComponent as Toolbar, ToolbarButton, ToolbarSeparator, ToolbarInput, ToolbarGroup, ToolbarLink };
