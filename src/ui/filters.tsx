...
export function toFilterObject(items: Filter[]): Record<string, unknown> {
	return items.reduce<Record<string, unknown>>((acc, item) => {
		...
		acc[item.field] = singleValueOperators.includes(item.operator) && Array.isArray(item.values) ? (item.values?.[0] ?? undefined) : item.values;
		return acc;
	}, {});
}
...
export const filterInputVariants = cvaWithMeta(
	[
		...
		'has-[[data-slot=filters-input]:focus-visible]:ring-2',
	...
	],
	{
		variants: {
			variant: {
				solid: 'border-0 bg-secondary-foreground',
...
export const filterRemoveButtonVariants = cvaWithMeta(
	['inline-flex items-center shrink-0 justify-center transition text-muted-foreground hover:text-foreground'],
	{
		variants: {
			variant: {
				solid: 'bg-secondary',
				outline: 'border border-border border-s-0 bg-background hover:bg-secondary',
			},
			size: {
...
export const filterAddButtonVariants = cvaWithMeta(
	[
...
export const filterFieldLabelVariants = cvaWithMeta(
	[
	...
	],
	{
		variants: {
			variant: {
				solid: 'bg-secondary',
				outline: 'border border-border border-e-0 bg-background',
			},
			size: {
				lg: 'h-10 px-4 text-sm gap-1.5 [&_svg:not([class*=size-])]:size-4',
				md: 'h-9 px-3 gap-1.5 text-sm [&_svg:not([class*=size-])]:size-4',
...
export const filterFieldValueVariants = cvaWithMeta(
	'text-foreground pe-0 ps-0 min-w-40 px-0 transition shrink-0 flex items-center gap-1 relative focus-visible:z-1 last:rounded-e-md',
	{
		variants: {
			variant: {
				solid: 'bg-secondary',
				outline: 'bg-background border border-border has-data-[slot=switch]:hover:bg-transparent',
			},
			size: {
				lg: 'h-10 text-sm gap-1.5 [&_svg:not([class*=size-])]:size-4',
				md: 'h-9 gap-1.5 text-sm [&_svg:not([class*=size-])]:size-4',
				sm: 'h-8 gap-1.25 px-2 text-xs [&_svg:not([class*=size-])]:size-3.5',
			},
			cursorPointer: {
...
export const filterFieldAddonVariants = cvaWithMeta('text-foreground shrink-0 flex items-center justify-center', {
	variants: {
		variant: {
			solid: '',
...
export const filterFieldBetweenVariants = cvaWithMeta('text-muted-foreground shrink-0 flex items-center', {
	variants: {
		variant: {
			solid: 'bg-secondary',
...
export const filtersContainerVariants = cvaWithMeta('flex flex-wrap items-center', {
	variants: {
		variant: {
			solid: 'gap-2',
			outline: '',
...
export const filterItemVariants = cvaWithMeta('flex items-center shadow-xs shadow-black/5', {
	variants: {
		variant: {
			solid: 'gap-px',
			outline: '',
...
function FilterInput<T = unknown>({
	...
	const getValidationMessage = (fieldType: string, hasCustomPattern = false): string => {
	...
	return (
		<div
			className={cn('min-w-40 px-4', filterInputVariants({ variant: context.variant, size: context.size }), className)}
			...
			<div className="w-full flex items-stretch">
				<input
					className="w-full outline-none placeholder-muted-foreground"
...
const createOperatorsFromI18n = (i18n: FilterI18nConfig): Record<FilterType, FilterOperator[]> => ({
	select: [
		{ value: 'is', label: i18n.operators.is },
		{ value: 'isNot', label: i18n.operators.isNot },
		{ value: 'empty', label: i18n.operators.empty },
		{ value: 'notEmpty', label: i18n.operators.notEmpty },
	],
	multiselect: [
		{ value: 'isAnyOf', label: i18n.operators.isAnyOf },
		{ value: 'isNotAnyOf', label: i18n.operators.isNotAnyOf },
		{ value: 'includesAll', label: i18n.operators.includesAll },
		{ value: 'excludesAll', label: i18n.operators.excludesAll },
		{ value: 'empty', label: i18n.operators.empty },
		{ value: 'notEmpty', label: i18n.operators.notEmpty },
	],
	date: [
	...
	daterange: [
		...
		{ value: 'notBetween', label: i18n.operators.notBetween },
		{ value: 'empty', label: i18n.operators.empty },
		{ value: 'notEmpty', label: i18n.operators.notEmpty },
	],
	daterangeinput: [
	...
	text: [
		...
		{ value: 'notContains', label: i18n.operators.notContains },
		{ value: 'startsWith', label: i18n.operators.startsWith },
		{ value: 'endsWith', label: i18n.operators.endsWith },
	...
	number: [
		...
		{ value: 'notEquals', label: i18n.operators.notEquals },
		{ value: 'greaterThan', label: i18n.operators.greaterThan },
		{ value: 'lessThan', label: i18n.operators.lessThan },
		{ value: 'between', label: i18n.operators.between },
		{ value: 'empty', label: i18n.operators.empty },
		{ value: 'notEmpty', label: i18n.operators.notEmpty },
	],
...
function FilterOperatorDropdown<T = unknown>({ field, operator, values, onChange }: FilterOperatorDropdownProps<T>) {
	...
	return (
		<DropdownMenu>
			<DropdownMenu.Trigger className={filterOperatorVariants({ variant: context.variant, size: context.size })}>
				{operatorLabel}
...
function SelectOptionsPopover<T = unknown>({ field, values, onChange, onClose, inline = false }: SelectOptionsPopoverProps<T>) {
	...
	const selectedOptions =
		field.type == 'autocomplete' && (field.options?.length ?? 0) < 1 && typeof values == 'string'
			? [{ value: values as unknown as T, label: values as unknown as string }]
		...
		return (
			<div className="w-full">
				<Command>
					...
					<Command.List>
						<Command.Empty>{field.noResultsFound || context.i18n.noResultsFound}</Command.Empty>

						{/* Selected items */}
						{selectedOptions.length > 0 && (
							<Command.Group heading={field.label || 'Selected'}>
						...
						{unselectedOptions.length > 0 && (
							<>
								{selectedOptions.length > 0 && <Command.Separator />}
								<Command.Group>
	...
	return (
		<Popover
			...
			<Popover.Content
				align="start"
				className={cn('w-50 p-0', field.className)}>
				<Command>
					{field.searchable !== false && (
						<Command.Input
							autoFocus
					...
					<Command.List>
						<Command.Empty>{context.i18n.noResultsFound}</Command.Empty>
...
function FilterValueSelector<T = unknown>({
			...
			<div
				...
				<div className="flex items-center gap-2">
					<Switch
						...
						size="sm"
					/>
					{field.onLabel && field.offLabel && <span className="text-xs text-muted-foreground">{isChecked ? onLabel : offLabel}</span>}
				</div>
			</div>
		);
	}
			...
			return (
				<div
					...
					<FilterInput
						...
						onInputChange={field.onInputChange}
						className={cn('min-w-40', field.className)}
						field={field}
					/>
					<div
						data-slot="filters-between"
	...
	if (field.type === 'select' || field.type === 'autocomplete' || field.type === 'multiselect') {
		return (
	...
	return (
		<Popover
			...
			<Popover.Trigger
				...
				<div className="flex gap-1.5 px-3 items-center">
					{field.customValueRenderer ? (
						field.customValueRenderer(values, field.options || [])
					) : (
						<>
			...
			<Popover.Content className={cn('min-w-60 p-0', field.popoverContentClassName)}>
				<Command>
					{field.searchable !== false && (
						<Command.Input
							autoFocus
...
interface FiltersContentProps<T = unknown> {
	...
	onChange: (filters: Filter<T>[], values: Record<string, unknown>) => void;
	withoutOperator?: boolean;
	className?: string;
}

export const FiltersContent = <T = unknown,>({ filters, fields, onChange, withoutOperator, className }: FiltersContentProps<T>) => {
	const context = useFilterContext();
		...
		(filterId: string, updates: Partial<Filter<T>>) => {
			const newFilters = filters.map((filter) => {
				if (filter.id === filterId) {
					...
					if (updates.operator === 'empty' || updates.operator === 'notEmpty') {
			...
			onChange(newFilters, toFilterObject(newFilters));
		},
		[filters, onChange],
	);
	...
	return (
		<div className={cn(filtersContainerVariants({ variant: context.variant, size: context.size }), context.className, className)}>
			{filters.map((filter) => {
				...
				return (
					<div
						...
						/>
						{/* Remove Button */}
						{!field.withoutRemoveButton && <FilterRemoveButton onClick={() => removeFilter(filter.id)} />}
					</div>
				);
			})}
		</div>
...
export function Filters<T = unknown>({
	...
	const addFilter = useCallback(
		(fieldKey: string) => {
			...
			if (field && field.key) {
				...
				const defaultOperator: OperatorKey =
					field.defaultOperator ||
					(field.type === 'daterange' ? 'between' : field.type === 'numberrange' ? 'between' : field.type === 'boolean' ? 'is' : 'is');
				...
				const newFilter = createFilter<T>(fieldKey, defaultOperator, defaultValues as T[], field.type);
	...
	const addFilterWithOption = useCallback(
		(field: FilterFieldConfig<T>, values: unknown[], closePopover = true) => {
			...
			const defaultOperator: OperatorKey = field.defaultOperator || (field.type === 'multiselect' ? 'isAnyOf' : 'is');
			...
			if (existingFilterIndex >= 0) {
				...
				onChange(updatedFilters, toFilterObject(updatedFilters));
			} else {
				// Create new filter
				const newFilter = createFilter<T>(field.key, defaultOperator, values as T[], field.type);
				const newFilters = [...filters, newFilter];
				onChange(newFilters, toFilterObject(newFilters));
			}
			...
			<div className={cn(filtersContainerVariants({ variant, size }), className)}>
				{showAddButton && selectableFields.length > 0 && (
					<Popover
						...
						/>
						<Popover.Content
							className={cn('w-full max-w-md p-0', popoverContentClassName)}
							align="start">
							<Command>
								{selectedFieldForOptions ? (
									...
									<SelectOptionsPopover<T>
										...
										onChange={(values) => {
											...
											const shouldClosePopover = selectedFieldForOptions.type === 'select' || selectedFieldForOptions.type === 'autocomplete';
									...
									<>
										...
										<Command.List className={'mt-2'}>
											<Command.Empty>{mergedI18n.noFieldsFound}</Command.Empty>
											{fields.map((item, index) => {
													...
													return (
														<Command.Group
															...
															{groupFields.map((field, fieldIndex) => {
																// Handle separator
																if (field.type === 'separator') {
																	return <Command.Separator key={`separator-${fieldIndex}`} />;
																}
												...
												// Handle separator
												if (field.type === 'separator') {
													return <Command.Separator key={`separator-${index}`} />;
												}
...
export const createFilter = <T = unknown,>(field: string, operator: OperatorKey = 'is', values: T[] = [], type: FilterType): Filter<T> => ({
